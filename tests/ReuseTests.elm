...
listR   = List   := static ""
readR   = Read   := int
createR = Create := static "new"
updateR = Update := int <> "update"
deleteR = Delete := int <> "delete"

opRoutes = router [listR, readR, createR, updateR, deleteR]
renderOp o =
  case o of
    List ()   -> reverse listR   []
    Read id   -> reverse readR   [toString id]
    Create () -> reverse createR []
    Update id -> reverse updateR [toString id]
    Delete id -> reverse deleteR [toString id]
...
articleR = child Article opRoutes
bookR    = "books" <//> child Book opRoutes
userR    = "users" <//> child User opRoutes

match    = Route.match <| router [articleR, bookR, userR]
render r =
  case r of
    Article op -> renderOp op
    Book op    -> reverse bookR [] ++ renderOp op
    User op    -> reverse userR [] ++ renderOp op
