...
import Route.QueryString as QS
...
view address model =
  let
    link' action content =
      a
        [ href "javascript:;", onClick address action ]
        [ text content ]

    link route = link' (UpdatePath route)
    linkQS route qs = link' (UpdatePathQS route qs)
  in
    div
      []
      [ ul [] [ li [] [ link (HomeR ()) "Home" ]
              , li [] [ link (PostsR ()) "Posts" ]
              , li [] [ linkQS (PostsR ()) (QS.empty |> QS.add "limit" "5") "More Posts" ]
