...
Elm.Data.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var posts = _U.list([{id: 1
                        ,title: "First Post"
                        ,body: "This is the first post"}
                       ,{id: 2,title: "Second Post",body: "This is the second post"}]);
   var lookupPost = function (id) {
      return $List.head(A2($List.filter,
      function (p) {
         return _U.eq(p.id,id);
      },
      posts));
   };
...
Elm.Routes.make = function (_elm) {
   ...
   AboutR,
   $Route.$static("about"));
   var PostR = function (a) {    return {ctor: "PostR",_0: a};};
   var postR = A2($Route._op[":="],
   PostR,
   A2($Route._op["<//>"],"posts",$Route.$int));
   ...
   var sitemap = $Route.router(_U.list([homeR
                                       ,postsR
                                       ,postR
                                       ,aboutR]));
   ...
   var route = function (r) {
      var _p0 = r;
      switch (_p0.ctor)
      {case "HomeR": return A2($Route.reverse,homeR,_U.list([]));
         case "PostsR": return A2($Route.reverse,postsR,_U.list([]));
         case "PostR": return A2($Route.reverse,
           postR,
           _U.list([$Basics.toString(_p0._0)]));
         default: return A2($Route.reverse,aboutR,_U.list([]));}
   };
...
Elm.Update.make = function (_elm) {
   ...
   var Home = {ctor: "Home"};
   var routeToPage = function (r) {
      var _p0 = r;
      switch (_p0.ctor)
      {case "HomeR": return Home;
         case "PostsR": return Posts($Data.posts);
         case "PostR": return A2($Maybe.withDefault,
           NotFound,
           A2($Maybe.map,Post,$Data.lookupPost(_p0._0)));
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return {ctor: "_Tuple2"
                           ,_0: model
                           ,_1: $Effects.none};
         case "PathChange": var path = A2($String.dropLeft,
           $String.length(pathPrefix),
           _p1._0);
           var page = function () {
              var _p2 = $Routes.match(path);
              if (_p2.ctor === "Nothing") {
                    return NotFound;
                 } else {
                    return routeToPage(_p2._0);
                 }
           }();
           return {ctor: "_Tuple2",_0: {page: page},_1: $Effects.none};
         default: return {ctor: "_Tuple2"
                         ,_0: model
                         ,_1: $Effects.task(A2($Task.map,
                         $Basics.always(NoOp),
                         $Task.toMaybe($History.setPath(A2(F2(function (x,y) {
   ...
   var init = function (path) {
      return {ctor: "_Tuple2"
             ,_0: {page: Home}
             ,_1: $Effects.task($Task.succeed(PathChange(path)))};
   };
...
Elm.View.make = function (_elm) {
   ...
   var _op = {};
   var posts = F2(function (link,ps) {
      var postLink = function (post) {
         return A2($Html.li,
         _U.list([]),
         _U.list([A2(link,$Routes.PostR(post.id),post.title)]));
      };
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Posts")]))
              ,A2($Html.ul,_U.list([]),A2($List.map,postLink,ps))]));
   });
   var post = function (post) {
      ...
      _U.list([A2($Html.h1,
              _U.list([]),
              _U.list([$Html.text(post.title)]))
              ,A2($Html.p,_U.list([]),_U.list([$Html.text(post.body)]))]));
   };
   var about = A2($Html.h1,
   _U.list([]),
   _U.list([$Html.text("About")]));
   var home = function (link) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.h1,
              _U.list([]),
              _U.list([$Html.text("Home Page")]))
              ,A2($Html.p,
              _U.list([]),
              _U.list([A2(link,
              $Routes.PostR(123),
   ...
   var notFound = A2($Html.h1,
   _U.list([]),
   _U.list([$Html.text("Page not found")]));
   var view = F2(function (address,model) {
      var link = F2(function (route,content) {
         return A2($Html.a,
         _U.list([$Html$Attributes.href("javascript:;")
                 ,A2($Html$Events.onClick,address,$Update.UpdatePath(route))]),
      ...
      _U.list([A2($Html.ul,
              ...
              _U.list([A2($Html.li,
                      _U.list([]),
                      _U.list([A2(link,$Routes.HomeR({ctor: "_Tuple0"}),"Home")]))
                      ,A2($Html.li,
                      _U.list([]),
                      _U.list([A2(link,$Routes.PostsR({ctor: "_Tuple0"}),"Posts")]))
                      ,A2($Html.li,
                      _U.list([]),
                      _U.list([A2(link,
                      $Routes.AboutR({ctor: "_Tuple0"}),
                      "About")]))]))
              ,function () {
                 var _p0 = model.page;
                 switch (_p0.ctor)
                 {case "Home": return home(link);
                    case "Posts": return A2(posts,link,_p0._0);
                    case "Post": return post(_p0._0);
...
Elm.Main.make = function (_elm) {
   ...
   var app = $StartApp.start({init: $Update.init(path)
                             ,view: $View.view
